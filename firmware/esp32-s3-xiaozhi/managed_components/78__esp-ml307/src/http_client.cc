...
void HttpClient::OnTcpData(const std::string& data) {
    ...
    if (rx_buffer_.size() + data.size() > MAX_RX_BUFFER_SIZE) {
        ESP_LOGE(TAG, "Receive buffer overflow: %zu + %zu > %zu",
                 rx_buffer_.size(), data.size(), MAX_RX_BUFFER_SIZE);
        SetError();
        connected_ = false;
        tcp_->Disconnect();
        return;
    ...
    while (!rx_buffer_.empty() && parse_state_ != ParseState::COMPLETE) {
        switch (parse_state_) {
            ...
            case ParseState::CHUNK_DATA: {
                ...
                // 当前 chunk 已接收完毕，等待并跳过 CRLF
                if (rx_buffer_.size() >= 2 && rx_buffer_.substr(0, 2) == "\r\n") {
                    rx_buffer_.erase(0, 2);
                    parse_state_ = ParseState::CHUNK_SIZE;
                } else if (!rx_buffer_.empty() && rx_buffer_[0] == '\r') {
                    return;  // 等待剩余的 LF
                } else if (!rx_buffer_.empty() && rx_buffer_[0] == '\n') {
